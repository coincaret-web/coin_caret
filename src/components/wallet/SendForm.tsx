...
export function SendForm({ availableBalance, senderAddress }: SendFormProps) {
  ...
  const handleOpenReview = (e: React.FormEvent) => {
    ...
    if (numTotal > numAvailable) {
      setError(`Insufficient available balance. Transfer amount (${numAmount} CC) + Gas Fee (${networkFee} CC) exceeds available ${availableBalance} CC.`);
      return;
    }
    ...
    try {
      ...
      const res = await fetch("/api/wallet/send", {
        ...
        body: JSON.stringify({
          ...
          note: "Direct CC Wallet Transfer",
        }),
      });
    ...
    <div className="max-w-xl mx-auto space-y-6">
      ...
      <div className="p-8 sm:p-10 rounded-3xl bg-slate-900/80 border border-slate-800/80 backdrop-blur-2xl shadow-2xl relative overflow-hidden">
        ...
        <p className="text-xs text-slate-400 mb-8">
          Transfer CC instantly with deterministic 10-second block finality.
        </p>
          ...
          <form onSubmit={handleOpenReview} className="space-y-6">
            ...
            <div>
              ...
              <span className="text-[11px] text-slate-500 mt-1 block">
                Format: CC0x + 40 hex characters with SHA-256 checksum
              </span>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="text-xs text-slate-400">
                  Available: <span className="font-mono text-emerald-400">{availableBalance} CC</span>
                </div>
              </div>
            ...
            <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800 space-y-2 text-xs">
              <div className="flex items-center justify-between text-slate-400">
                ...
                <span className="font-mono">{networkFee} CC</span>
              </div>
